...
D = 20000;
[TL,TLc,TLt] = TransmissionLoss_wDepth(f/1000, R,D);
% TL = TransmissionLoss(f/1000, R, 's');
...
%%
figure
legendinfo = cell(1,length(R)*2);
semilogx(f/1000, SE_r0)
for idx = 1:length(R)
    legendinfo{idx} = ['R = ' num2str(R(idx)/1e3) ' kyds Spherical Spreading'];
end
hold on
semilogx(f/1000, SEc_r0,'--');
for idx = 1:length(R)
    legendinfo{length(R)+idx} = ['R = ' num2str(R(idx)/1e3) ' kyds Cylindrical Spreading'];
end
legend(legendinfo,'location','Northwest','FontSize',18)
v = axis;
axis([v(1:3) v(4)+10])
xlabel('f, KHz','Fontsize',18)
ylabel('SE, dB // {\mu}Pa/Hz @1 yd','Fontsize',18)
title(['Signal Excess'],'FontSize',18)
...
[TLs,TLc,TLt1] = TransmissionLoss_wDepth(F/1000, R,D1);

[~,~,TLt2] = TransmissionLoss_wDepth(F/1000, R,D2);
figure;
semilogx(F,TLc(:,end),F,TLt1(:,end),F,TLt2(:,end),F,TLs(:,end));
legend({'Cylindrical Spreading','Deep case, D = 4 kyds','Shallow case, D = 100 yds','Spherical Spreading'},'FontSize',18); 
title(['R = ' num2str(R(end)/1e3) ' kyds'],'FontSize',18);xlabel('Frequency','FontSize',18);ylabel('TL, dB','FontSize',18);
figure;semilogx(F,TLc(:,1000),F,TLt1(:,1000),F,TLt2(:,1000),F,TLs(:,1000));
legend({'Cylindrical Spreading','Deep case, D = 4 kyds','Shallow case, D = 100 yds','Spherical Spreading'},'FontSize',18); 
title(['R = ' num2str(R(1000)/1e3) ' kyds'],'FontSize',18);xlabel('Frequency','FontSize',18);ylabel('TL, dB','FontSize',18);
