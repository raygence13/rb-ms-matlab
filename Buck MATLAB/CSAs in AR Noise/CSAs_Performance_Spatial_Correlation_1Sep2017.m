...
[derived.CSApp.PTS,SDT] = CSAProdPowTermsSeq(L,I_M,I_N,M,N,beta,u0);
...
for a_ind = 1:length(alpha)
    ...
    derived.CSApp.OutNoisePow(1,a_ind) = ...
        (wM'*wN)/beta*varW*sum(derived.CSApp.coarray.*alpha(a_ind).^abs(gamma).*exp(1i*pi.*gamma*u0));
...
%     for plotind = 1:length(zeta)
        ...
        line_fewer_markers(u,10*log10(abs(simulated.ULA.NoisePSD{1,aplot})),50,'r:^','MarkerSize',8,'Spacing','x','lockonmax',1);
        
        plot(u,10*log10(abs(derived.CSAcbf.NoisePSD{1,aplot})),'color',plotcolor.CSAcbf)
        line_fewer_markers(u,10*log10(abs(simulated.CSAcbf.NoisePSD{1,aplot})),50,':+','MarkerSize',12,'color',plotcolor.CSAcbf,'Spacing','x','lockonmax',1);
        
        plot(u,10*log10(abs(derived.CSApp.NoisePSD{1,aplot})),'color',plotcolor.CSApp)
        line_fewer_markers(u,10*log10(abs(simulated.CSApp.NoisePSD{1,aplot})),150,'k:x','MarkerSize',12,'Spacing','x','lockonmax',1);
...
for aplot = 5
    ...
    clf
    hold on
    sigline = plot([u0 u0],[-50 50].','-.m');
    zsigder = zeros(1,length(zeta));
%     for plotind = 1:length(zeta)
    simSigNoisePSD = 10*log10(abs(simulated.CSAcbf.SigNoisePSD{1,aplot}(SigindOfInterest)));
    simRoS = 10*log10(abs(simulated.CSAcbf.SigNoisePSD{1,aplot}(RightOfSig)));
    simLoS = 10*log10(abs(simulated.CSAcbf.SigNoisePSD{1,aplot}(LeftOfSig)));
...
%     plot(u,10*log10(abs(simulated.(zeta{plotind}).SigNoisePSD{1,aplot})),':','color',plotcolor.sim)
    zsigsim(1) = plot(uSig(1:3:end),simSigNoisePSD(1:3:end),':+','MarkerSize',12,'LineWidth',2.5,'color',plotcolor.CSAcbf);
    plot(uRofS(1:10:end),simRoS(1:10:end),':+','MarkerSize',12,'LineWidth',2.5,'color',plotcolor.CSAcbf)
    plot(uLofS(1:10:end),simLoS(1:10:end),':+','MarkerSize',12,'LineWidth',2.5,'color',plotcolor.CSAcbf)
    
    
    simSigNoisePSD = 10*log10(abs(simulated.CSApp.SigNoisePSD{1,aplot}(SigindOfInterest)));
    simRoS = 10*log10(abs(simulated.CSApp.SigNoisePSD{1,aplot}(RightOfSig)));
    simLoS = 10*log10(abs(simulated.CSApp.SigNoisePSD{1,aplot}(LeftOfSig)));
    
    zsigder(2) = plot(u,10*log10(abs(derived.CSApp.SigNoisePSD{1,aplot})),'color',plotcolor.CSApp);
    
%     plot(u,10*log10(abs(simulated.(zeta{plotind}).SigNoisePSD{1,aplot})),':','color',plotcolor.sim)
    zsigsim(2) = plot(uSig(1:3:end),simSigNoisePSD(1:3:end),':x','MarkerSize',15,'LineWidth',2.5,'color',plotcolor.CSApp);
    plot(uRofS(1:10:end),simRoS(1:10:end),':x','MarkerSize',15,'LineWidth',2.5,'color',plotcolor.CSApp)
    plot(uLofS(1:10:end),simLoS(1:10:end),':x','MarkerSize',15,'LineWidth',2.5,'color',plotcolor.CSApp)
    
    
%     end
    tpsd = plot(u,10*log10(abs(derived.ULA.SigNoisePSD{1,aplot})),'-.','LineWidth',2.5,'color',plotcolor.truePSD);
...
%% ROC plots
load ULAdata_SNR_20dB.mat;
derived.ULA = derived_ULA_SNR_20;
simulated.ULA = simulated_ULA_SNR_20;
if doROC
f12 = figure(12);
...
%         ylim([log10(min(simulated.CSAcbf.Pd{1,ROC1}))-.01 0])
        ...
        loglog((abs(derived.ULA.Pfa{1,ROC1})),(abs(derived.ULA.Pd{1,ROC1})),'color',plotcolor.ULA)
        hold on
        loglog((simulated.ULA.Pfa{1,ROC1}(1:skip1:end-endskip)),(simulated.ULA.Pd{1,ROC1}(1:skip1:end-endskip)),':s','MarkerSize',10,'color',plotcolor.ULA)
        loglog((abs(derived.CSAcbf.Pfa{1,ROC1})),(abs(derived.CSAcbf.Pd{1,ROC1})),'color',plotcolor.CSAcbf)
        loglog((simulated.CSAcbf.Pfa{1,ROC1}(1:skip1:end-endskip)),(simulated.CSAcbf.Pd{1,ROC1}(1:skip1:end-endskip)),':s','MarkerSize',10,'color',plotcolor.CSAcbf)
        loglog((abs(derived.CSApp.Pfa{1,ROC1})),(abs(derived.CSApp.Pd{1,ROC1})),'color',plotcolor.CSApp)
        loglog((simulated.CSApp.Pfa{1,ROC1}(1:skip1:end-endskip)),(simulated.CSApp.Pd{1,ROC1}(1:skip1:end-endskip)),':s','MarkerSize',10,'color',plotcolor.CSApp)
