...
rho = abs(rho);
q = x./(sigmaM*sigmaN*(1-rho^2));
BesselIterm = besseli(0,q*abs(rho));
BesselKterm = besselk(0,q);

Z = q./(sigmaM*sigmaN).*BesselIterm.*BesselKterm;
...
%     newBesselterm = exp(x(ABind).*(rho-1))./(2.*x(ABind).*sqrt(rho));
    Z(ABind) = exp(q(ABind).*(rho-1))./(2*sigmaM*sigmaN*sqrt(rho));
end
end
