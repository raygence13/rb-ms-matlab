...
plotcolor.CSAcbf    = [0.17 0.51 0.34];         % CSAcbf: orange
...
Me = sum(derived.M.ArrayPos);  % number of sensors in M-subarray

derived.N.ArrayPos = zeros(1,L);
derived.N.ArrayPos(1:N:end) = 1;
Ne = sum(derived.N.ArrayPos);  % number of sensors in N-subarray

% combined subarrays/coprime array
derived.CSAcbf.ArrayPos = double(or(derived.M.ArrayPos,derived.N.ArrayPos));
Lambda = sum(derived.CSAcbf.ArrayPos); % number of unique sensors in CSA
...
rM_Sig = rM_Sig/Me;
rN_Sig = rN_Sig/Ne;
rCSAcbf_Sig = rCSAcbf_Sig/Lambda;
rCSAcbfsteered_Sig = rCSAcbfsteered_Sig/Lambda;
...
derived.CSAcbf.Beampattern = fftshift(fft(rCSAcbf_Sig/Lambda/varS,uNfft));
derived.CSAcbf.Beampatternsteered = fftshift(fft(rCSAcbfsteered_Sig/Lambda,uNfft));
...
[derived.CSApp.PTS,SDT] = CSAProdPowTermsSeq(L,Me,Ne,M,N,beta,u0);

for a_ind = 1:length(alpha)
    %% Alpha portion
    threshold = -10;     % Sets value for extrasamples/sensors for steady state
    [b,a,extrasamples] = ARProcessCoefficients( alpha(a_ind),1,threshold );
    % y[l] - alpha^USF*y[l-USF] = x[l]
    % a = [1 zeros(1,USF-1) -alpha^(USF)];
    % b = 1
    
    SampleSize = 5000;  % number of snapshots
...
%     truePSDNoise{1,a_ind} = (b*sqrt(1-alpha(a_ind)^2)./(a(1) + a(2).*exp(-1i*pi.*u))).^2*varW;
    truePSDNoise{1,a_ind} = (b*(1-alpha(a_ind)^2)./(a(1) + alpha(a_ind)^2 - 2*alpha(a_ind).*cos(u*pi)))*varW;
    ...
    rCSAcbf_Noise = derived.CSAcbf.coarray.*alpha(a_ind).^abs(gamma)/Lambda*varW;
    ...
    rCSAcbf_simNoise = rCSAcbf_simNoise/Lambda;
    ...
    rCSAcbf_simSig = rCSAcbf_simSig/Lambda;
    ...
    rM_simSigNoise = rM_simSigNoise/Me;
    rN_simSigNoise = rN_simSigNoise/Ne;
    rCSAcbf_simSigNoise = rCSAcbf_simSigNoise./Lambda;
    ...
    derived.M.OutNoisePow(1,a_ind) = ...
        (wM'*wM)/Me*varW*sum(derived.M.coarray.*alpha(a_ind).^abs(gamma).*exp(1i*pi.*gamma*u0));
    ...
    derived.N.OutNoisePow(1,a_ind) = ...
        (wN'*wN)/Ne*varW*sum(derived.N.coarray.*alpha(a_ind).^abs(gamma).*exp(1i*pi.*gamma*u0));
    ...
    derived.CSAcbf.OutNoisePow(1,a_ind) = ...
        (wCSA'*wCSA)/Lambda*varW*sum(derived.CSAcbf.coarray.*alpha(a_ind).^abs(gamma).*exp(1i*pi.*gamma*u0));
    ...
    %%%%------------------------------------------------------------------------------------------------
    derived.CSApp.PowOutNoisePow(1,a_ind) = (1/Me^2/Ne^2)*varW^2*...
...
% derived.ULA = derived_M2N5B10_denseAG.ULA;
...
%     semilogx(1-alpha,10*log10(abs(derived.M.AG(b,:))),'color',plotcolor.M)
%     semilogx(1-alpha,10*log10(abs(simulated.M.AG(b,:))),':^','color',plotcolor.M)
%     semilogx(1-alpha,10*log10(abs(derived.N.AG(b,:))),'color',plotcolor.N)
%     semilogx(1-alpha,10*log10(abs(simulated.N.AG(b,:))),':^','color',plotcolor.N)
...
%     semilogx(1-alpha,10*log10(abs(CSACBFAGder)),'color',plotcolor.CSAcbf)
%     semilogx(1-alpha,10*log10(abs(CSACBFAGsim)),':^','color',plotcolor.CSAcbf)
%     semilogx(1-alpha,10*log10(abs(CSAPPAGder)),'color',plotcolor.CSApp)
%     semilogx(1-alpha,10*log10(abs(CSAPPAGsim)),':^','color',plotcolor.CSApp)
    grid on
    ax = gca;
