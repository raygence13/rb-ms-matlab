...
% p = [(0:Nsens-1)'*d...      % sensors along x-axis
%     zeros(Nsens,1)...
...
for block = 1:numBlocks
    ...
    for indf = 1:l_f0 % Outputs are H/VS_DEvsAZ, H/VS_FRAZ
        ...
        X_snaps = X_p(:,f0idx(indf)); %X_snaps = X_snaps/max(abs(X_snaps));
        Y_snaps = Y_p(:,f0idx(indf)); %Y_snaps = Y_snaps/max(abs(Y_snaps));
        Z_snaps = Z_p(:,f0idx(indf)); %Z_snaps = Z_snaps/max(abs(Z_snaps));
        Snaps   = [H_snaps;X_snaps;Y_snaps;Z_snaps];
